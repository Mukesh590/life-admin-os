...
import { formatDate, getDaysUntil, isExpiringSoon, cn } from '@/lib/utils'
...
export function WarrantiesClient({ initialData, userId }: Props) {
  ...
  useEffect(() => {
    ...
    setForm(current => ({
      ...current,
      product_name: title,
      coverage_notes: searchParams.get('captureNote') || '',
      expiry_date: searchParams.get('captureDue') || '',
    }))
    setShowForm(true)
  }, [searchParams])
          ...
          {warranties.map(w => {
            ...
            return (
              ...
              >
                ...
                <UrgencyBadge date={w.expiry_date} className="shrink-0" />
